...
- `scripts/build_alina_job_manual.py`：一次性生成阿离岗位投递手册，抓智联公开岗位，按简历画像筛选，输出带城市/薪资/实习全职筛选的 HTML、PDF/DOCX、岗位 JSON 和平台反爬策略 JSON。
...
- HTML 手册必须能在同一页完成城市、薪资高低、实习/全职筛选；投递链接必须新开页面，避免打开岗位后覆盖手册。
